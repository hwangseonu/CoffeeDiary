<script lang="ts">
  ...
  $: gradientString = notes.length > 0 ? notes.map((v) => v.color).join(', ') : '#cccccc';
...
<div class="w-full max-w-full bg-white rounded-xl shadow-md p-5">
  ...
  <div class="p-4 bg-gray-50 rounded-xl shadow-2xl mx-auto max-w-lg">
    ...
    >
      ...
      <input
        bind:value={customNote.text}
        type="text"
        id="new-flavor-name"
        placeholder="저장할 노트 이름 (예: 플로럴, 트로피컬)"
  ...
  <div class="mt-4 pt-4 border-t border-gray-200">
    ...
    <div class="flex flex-col sm:flex-row gap-2">
      <div id="quick-notes-container" class="flex flex-wrap gap-2">
        {#each wellKnownNotes as note (note.text)}
          <button
            type="button"
            on:click={() => addNote(note)}
  ...
  <div class="pt-4 border-t border-gray-200">
    ...
    <div
      class="color-gradient-bar h-5 rounded-lg"
