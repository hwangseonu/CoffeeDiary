    ...
    {#each categories as category (category.id)}
      <div class="space-y-2">
        <div class="flex justify-between items-center text-xs">
          ...
          <span id="strength-val" class="font-bold text-amber-600"
            >{sensuality[category.id.toLocaleString().toLowerCase()]}</span
          >
        </div>
        <div class="slider-container">
          <input
            bind:value={sensuality[category.id.toLocaleString().toLowerCase()]}
            type="range"
            id="strength"
            min="0"
            max="5"
