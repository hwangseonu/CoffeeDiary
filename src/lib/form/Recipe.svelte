...
<div class="w-full bg-white rounded-xl p-5 shadow-md border border-stone-100">
    ...
    <div class="space-y-4 text-sm">
        ...
        <div class="grid grid-cols-3 gap-2 mt-2">
            ...
            <label>추출비율 (Ratio): <input value="1:{(recipe.extract/recipe.dose).toFixed(2).replace(/\.?0+$/, '').replace(/\.$/, '')}" type="text" id="ratio" class="w-full p-2 border border-stone-300 rounded-md" disabled></label>
            ...
            <label class="flex flex-col w-full col-span-full">추출변수(Dynamics)
                <textarea class="w-full h-80 rounded-md border border-stone-300 resize-none"></textarea>
            </label>
        </div>
    </div>
</div>